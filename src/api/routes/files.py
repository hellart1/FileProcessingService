...
@router.post('/files')
async def upload_file(uploaded_file: UploadFile, session: SessionDep):
...
async def get_file_status(file_id: str):
    data = json.loads(await redis.redis_client.get(file_id))
    return data

@router.get('/files/{file_id}/result')
async def get_file_result(file_id: str):
    data = json.loads(await redis.redis_client.get(f"{file_id}:result"))
    return data
